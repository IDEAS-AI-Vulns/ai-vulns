...
public class ConstraintController {
    ...
    @PreAuthorize("hasAuthority('ADMIN')")
    @GetMapping(value = "/api/v1/constraints/update/{force}")
    public ResponseEntity<String> updateConstraints(@PathVariable("force") boolean force) {
        try {
            updateVulnerabilityService.updateConstraintsForAllVulnerabilities(force);
